...
from CVAE import CVAE_model
from CVAE import Imageset
# import os
# import warnings
import numpy as np
...
class CVAE:
    ...
    def __init__(self, mode='new', n_dim=3, model_path='./ModelSaved'):
        ...
        self.font = {"color": "darkred",  # visualization font
                     "size": 13,
                     "family": "serif"}
    ...
    def train(self, save_path, n_epoch=1, batch_size=100, learning_rate=0.001, **kwargs):
        ...
        for epoch in range(n_epoch):
            ...
            for step, [train_batch, Labels] in enumerate(trainings):
                ...
                batch_loss, _ = self._train_vae(train_batch, Labels, **kwargs)
    ...
    @tf.function
    def _train_vae(self, data, lab, weight_catloss=3):
        with tf.GradientTape() as tape:
            reconstruct = self.model(data, training=True)
            loss = tf.reduce_sum(tf.square(data - reconstruct[0]))
            loss += tf.reduce_sum(self.model.losses)  # Add Dkl regularization loss
        ...
        grads = tape.gradient(loss, self.model.variables)  # all variables are trainable
        self.optimizer.apply_gradients(grads_and_vars=zip(grads, self.model.variables))
        self.loss_metric(loss)
        return self.loss_metric.result(), cat_loss
    ...
    def reconstruction(self, data_source='train', n_show=10):
        # random visualization of the reconstruction
        if data_source == 'train':
            rand_idx = np.random.randint(0, int(self.train_file_set.num_data / n_show), 1)
            # randomly choose a batch to show
            img_show = self.train_dataset.batch(n_show)
        elif data_source == 'test':
            rand_idx = np.random.randint(0, int(self.test_file_set.num_data / n_show), 1)
            img_show = self.test_dataset.batch(n_show)
        else:
            raise Exception("data_source must be 'train' or 'test'")

        for index, [data, label] in enumerate(img_show):
            if index == rand_idx:
                data = tf.cast(data, tf.float32)
                pred_img = self.model(data, training=False)
                ...
                f.show()
                plt.suptitle('VAE_images_reconstructed ()', fontdict=self.font)
                plt.draw()
                plt.show()

    def latent_space(self, latent_dims):
        ...
        plot_batch = self.train_dataset.batch(batch_size=100)  # take the trainset as plot data
        ...
        for data, labels in plot_batch:  # calculate the latent codes
            ...
            latent_code = self.model.encoder(data, training=False)
            x = tf.concat([x, latent_code[0]], 0)  # [0]: z-mean
        fig = plt.figure(figsize=(8, 8))
        ax = fig.add_subplot(111, projection='3d')
        ax.scatter(x.numpy()[1:self.train_file_set.num_data + 1, 0], x.numpy()[1:self.train_file_set.num_data + 1, 1],
                   x.numpy()[1:self.train_file_set.num_data + 1, 2],
                   c=self.train_file_set.labels, alpha=0.5, cmap=plt.cm.get_cmap('rainbow', 7))
        plt.title("Data Points in Latent Space", fontdict=self.font)
        plt.tight_layout()
